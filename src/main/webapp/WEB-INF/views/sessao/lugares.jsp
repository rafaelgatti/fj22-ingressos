							...
							<table>
								...
								<c:forEach var="lugar" items="${map.value}">
									<td class="fileira-assento"><figure>
										<svg onclick="${sessao.isDisponivel(lugar) && !carrinho.isSelecionado(lugar) ? 'changeCheckbox(this)' : ''}" class="assento ${sessao.isDisponivel(lugar) && !carrinho.isSelecionado(lugar) ? "disponivel" : "ocupado"}" id="${lugar.id}" title="${lugar.fileira}${lugar.posicao}"  version="1.0" id="SEAT" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
        ...
        <script>
            ...
            function changeCheckbox(img) {
                ...
                if (!checkbox.checked){
                    ...
                    var lugar = document.createTextNode($(img).attr('title'));
